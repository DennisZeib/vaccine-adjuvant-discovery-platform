...
def calculate_scores(final: Dict[str, float]) -> Tuple[Dict[str, VaccineScore], float, float]:
    ...
    (dose-response), όχι μόνο μία στατική τιμή αναφοράς.
    """
    scorer = get_scorer()
    scores: Dict[str, VaccineScore] = {}
    for met in TARGET_METABOLITES:
        concentration = final.get(met, 0.0)
        scores[met] = scorer.score_molecule(
            met, met, "", {"concentration": concentration}
        )
